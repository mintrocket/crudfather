	...
	class StatisticBuilderController extends CBController {
	    ...
	    public function getDashboard() {
	    	$this->cbLoader();
				...
				// FIX START
				$menus_privileges = DB::table('cms_menus_privileges')
				->where('id_cms_privileges',CRUDBooster::myPrivilegeId())
				->get();

				$conditions = $menus_privileges->map(function($row) { return $row->id_cms_menus; });
	    ...
	    public function getViewComponent($componentID) {
				...
				$component = DB::table('cms_statistic_components')->where('id', $componentID)->first();
				$date_from = Request::get('date_from');
				$date_to = Request::get('date_to');

				return response()->json(eval($component->config));
