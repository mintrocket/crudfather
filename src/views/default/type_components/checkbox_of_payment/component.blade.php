...
@push('bottom')
						...
						let order_price = 0;
						  function change_order_price(value) {
						    order_price = value;
						  }

						  $(()=>{

						    change_order_price($('#form-group-order_cost input').val())

						    $('input[name="{{$name}}[]"]').on('change', e => {

						      console.log(e.target.chcked)
						      $('option[value="Наличный расчет"]').prop('selected', e.target.checked);
						      $('#method_paid').prop('disabled', e.target.checked);
						      if(e.target.checked) {
						        $('#form-group-order_cost').hide(350);
						        $('#form-group-order_cost input').val('0');
						      } else {
						        $('#form-group-order_cost').show(350);
						        $('#form-group-order_cost input').val(order_price);
						      }
						    });

						    $('#form-group-order_cost input').on('change', e => {
						      change_order_price(e.target.value);
						    });
